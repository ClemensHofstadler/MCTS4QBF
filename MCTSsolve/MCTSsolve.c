...
static Var * lit2var (int lit) {
  if (!(lit && abs(lit) <= num_vars)) printf("num_vars = %d\n",num_vars);
...
static uint64_t * bits, * MCTS_bits;
static int reward, vars_left, rollout_flag, nr_outer_most_vars, len_assignment;
...
static void clear_assignment (void) {
    for(int i = 0; i < len_assignment; i++)
        MCTS_bits[i] = bits[i];
}
...
static void release_mcts (void) {
    ...
    DELN (bits,len_assignment);
    DELN (MCTS_bits,len_assignment);
...
int main (int argc, char ** argv) {
  ...
  len_assignment = ((int) log2(num_vars)) + 1;
  NEWN (bits,len_assignment);
  NEWN (MCTS_bits,len_assignment);
